...
const Discover = () => {
  ...
  const songCount = currentPage * 20;
  const { activeSong, isPlaying, genreListId } = useSelector((state) => state.player);
  const { data, isFetching, error } = useGetTopChartsByGenreQuery({ genreListId, songCount });
  const startIndex = (currentPage - 1) * 20;
  const endIndex = (currentPage * 20);
  const songs = data?.tracks?.slice(startIndex, endIndex);
